...
export default class CompanyStats extends Vue {
  ...
  onEmployeesChanged(value: string) {
    if (this.companyInfo && this.companyInfo["Employees"].length === value.length) {
      this.employees.forEach(emp => {
        ...
        this.locations[this.locations.findIndex(location => location.label === emp.location)].count += 1;
        this.ageCategories[this.ageCategories.findIndex(ageCategory => emp.age > ageCategory.min &&   emp.age <= ageCategory.max)].count += 1;
      })
    }
  }
}
